...
Two manifests you can copy into your data root. Nothing here needs a change to
...
|---|---|---|
| [`device-app`](device-app/) | 8479 | The whole loop, and then some: health row, live-discovered tools, `role: device`, and **push** ingest so the app hands Ava events when *it* decides. Start here. |
...
- **"How do I get anything working?"** - `device-app`. Ignore its `ingest:` block
  on a first read and what is left is the minimal loop: a manifest, a health
  probe, and a tool list Ava discovers for itself.
- **"How do I expose my own app's tools?"** - `device-app`, whose `actions.discover`
...
- **"How does my own hardware push readings in?"** - `device-app`, whose `ingest:`
  block declares the channels and whose README walks the bearer-token handshake.
