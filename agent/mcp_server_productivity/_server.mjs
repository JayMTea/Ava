...
const SERVER_INFO = { name: 'ava-tools-productivity', version: '2.0.0' };
const ctx = {
...
for (const full of collectModules(HERE)) {
  ...
  catch (e) { process.stderr.write(`[ava-tools-productivity] failed to load ${rel}: ${e.message}\n`); continue; }
  const t = mod.default;
  if (!t || !t.name || typeof t.handler !== 'function' || !t.inputSchema) {
    process.stderr.write(`[ava-tools-productivity] skipping ${rel}: must default-export { name, description, inputSchema, handler }\n`);
    continue;
  }
  if (TOOLS[t.name]) process.stderr.write(`[ava-tools-productivity] WARNING: duplicate tool name "${t.name}" (${rel}) overrides earlier\n`);
  TOOLS[t.name] = t;
}
process.stderr.write(`[ava-tools-productivity] loaded ${Object.keys(TOOLS).length} tool(s): ${Object.keys(TOOLS).join(', ')}\n`);
