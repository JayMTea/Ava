...
CI runs 11 jobs on every PR: `ruff`, `shellcheck`, the `tests/` tier, the
`qa/` tier, a frontend dist-drift check, a CPU-only smoke boot plus
`ava verify`, three compose checks (config, install, smoke), a secrets scan,
