...
Runs daily via systemd timer (ava-learning-digest.timer) at 4am PST.
"""
  ...
  <div class="container">
    <div class="email">
      <div class="header">
        ...
        <p>{now.strftime('%A, %B %d, %Y at %I:%M %p PST')}</p>
      </div>
      
      <div class="content">
        <div class="section">
          <h2><span class="icon">🔵</span> Code Learning</h2>
          {code_html}
          <a href="http://localhost:8096/" class="button">View Code Cycles</a>
        </div>
        
        <div class="section">
          <h2><span class="icon">🟠</span> Chat Learning</h2>
          {chat_html}
...
{now.strftime('%A, %B %d, %Y at %I:%M %p PST')}
...
def send_email(html_body, text_body):
    ...
    Requires .env with OUTLOOK_EMAIL and OUTLOOK_PASSWORD (app-specific password).
    """
    ...
    # mailbox on Outlook so a fork emails ITSELF, not the author.
    ...
    try:
        ...
        # Send via SMTP (default Office 365). Use an app-specific password.
