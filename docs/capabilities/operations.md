...
and **Control**, the Control Center where learning proposals and staged code
diffs are actually approved, rejected or thrown away.

The badge on the **Control** tab is the count of learning proposals still
waiting on a decision (the pending ones in the latest code cycle plus the
latest chat cycle). When it is empty, nothing is parked.

## How "live" actually works

There is **one** push channel: `GET /api/stream/ops`, a Server-Sent Events
stream (`text/event-stream`, `Cache-Control: no-cache`,
`X-Accel-Buffering: no`), cookie-gated like the rest of the `/api` surface. A
single async producer runs at **1 Hz**: it snapshots the shared runtime state,
diffs it against the previous tick, and emits **only what changed**. A `: keepalive`
comment goes out every 15 s so proxies don't reap an idle connection, and the
loop exits as soon as the client disconnects — closing the tab stops the work
on the server.
...
| `device.event` | A connected device pushed an event (see below). |
| `alert.raise` / `alert.clear` | Alerts are re-evaluated every 5th tick (~5 s); only transitions are sent. |

Device events are replayed **only from the sequence number current at connect
time** — opening the page never floods you with an hour of backlog.

> **There are zero WebSocket endpoints in Ava.** The live UI is this one SSE
> stream plus ordinary polling. Nothing else. The only `websocket` reference in
> the codebase is the the GPU service socket the bridge consumes *outbound* to get true
> render progress.

The polling half is a set of thin reads over the same aggregation layer, each
on its own cadence, all of them paused while the browser tab is hidden:
...
Poll *and* push live in one module (`ava_bridge/ops_api.py`) on purpose: they
are two halves of one contract, and splitting them is how an event type gets
added to one and forgotten in the other. The SSE producer would otherwise
hammer `systemctl` and health probes once a second, so the reads behind it are
TTL-cached server-side (services 15 s, timers 30 s, the raw perf scan 10 s).

If the stream drops, the client reconnects with exponential backoff; until it
does, the polled values still refresh the page. The stream is an accelerant,
not a dependency.
...
otherwise the step kind — plus the step and tool counts. A render row shows the
first 80 characters of the prompt with a live progress bar and percentage.
...
with severity `warn`/`critical` are spoken aloud by the browser's Web Speech
...
**Job queue** is a table of recent renders and upscales — Kind, Status, Source,
Progress, Started.
...
user timers, the panel honestly says *"No timers found."* rather than inventing
a schedule.
...
perf, egress-route count, action count), and **Tool usage** ranks the tools the
agent has actually called, counted across the turns currently in memory.

> **What is live is not what is durable.** Turns and jobs are in-process state:
> a stalled render is flipped to `error` after 15 minutes with an explanation,
> and terminal jobs are dropped an hour after they finish so the table can't
> grow forever. The permanent record is the append-only audit ledger — **Setup
> → History**, and see [Data, memory & privacy](data.md).

## Alerts

Alerts are a small rule engine over **`config/alerts.yaml`**, evaluated
in-process on every tick against a flat metrics dictionary. A rule is a handful
of fields:

```yaml
- id: gpu_temp
  metric: gpu_temp        # a key from the metrics table below
  op: ">"                 # >  <  >=  <=  ==
  value: 85               # threshold
  for: 30                 # debounce: seconds the breach must persist to raise
  cooldown: 120           # anti-flap: seconds before it may raise again
...
YAML is missing or unparseable the engine falls back to two built-in rules (GPU
temperature and service-down) rather than going silent, and the file is re-read
whenever its mtime changes.

**There is no external paging.** No webhook, no email, no third-party service.
Alerts are single-user and in-process: they surface in the Alerts panel, on the
critical banner, and as `alert.raise` / `alert.clear` frames on the SSE stream.
Nothing leaves the machine.

These are the metrics the engine computes each pass:
...
The budget and allocation metrics read **0 until you configure them** (a cap in
**Setup → Budgets**, models under `alloc.models`), so those rules sit dormant on
a fresh install instead of firing on defaults you never chose. How the
underlying spend, energy and throughput numbers are measured is on
[Vitals](vitals.md).

One more source feeds the same panel: a device connector's `notify`, `warn` or
`critical` event is injected as a short-lived external alert (90 s by default),
so an urgent reading gets a place in the active-alert list and not just a
transient toast.
...
Opening an app drills into its cycles. For Ava there are two tabs, **Code** and
**Chat**, matching the two proposal streams: edits to her own source, and
...
`+added` / `-removed` counts and a **view diff** toggle that opens a **per-file
unified diff viewer** — added, removed, hunk-header and context lines coloured
...
Nothing here applies itself. A cycle **parks** proposals; the apply call is the
only thing that writes, and the self-editing access policy still governs what
that call is allowed to touch — sensitive paths stage for approval, secrets and
`ava.yaml` are denied outright. That machinery is on [The agent](agent.md). If
`features.learning` is off the page says so plainly rather than promising
...
Connector consent has a sharper edge than proposal review, because the agent is
mid-call. When it invokes an action that requires your OK, the bridge **parks
the request and blocks the call** on a condition variable — the tool does not
return, and the agent does not proceed, until you decide. The numbers are
deliberate:

- **120-second timeout.** No answer is not a yes; the call comes back
  `timeout` and is refused.
- **50 parked requests maximum.** Past that, new requests **fail closed to
  denied** rather than queueing up an unbounded pile of things you might
  accidentally wave through.
- The wait uses a condition variable, so your click wakes the blocked call
  immediately — there is no polling on the server side.

The prompt itself is a banner on the **Setup** page (it polls
`GET /api/hub/approvals` every 3 s, so it finds you on whichever tab you are
on): *"Ava wants to run `<action>` on `<connector>`"*, with the arguments and,
for the harsher tiers, the reason it is asking — *destructive — asks every
time*, *physical action — moves something in the real world; asks every time*.

| Button | Decision | Effect |
|---|---|---|
| **Always allow** | `always` | Runs now **and** writes a durable grant so this action never asks again. Only offered where the action is grantable — write tier, no author-declared confirm. |
...
Grants live in `$AVA_HOME/connector_grants.yaml` and are revocable at any time
from the connector's permission sheet in **Setup → Connectors**.
...
and `revoke` events. "What did I approve, and when?" is answerable from **Setup
→ History** without trusting the UI's memory.
...
- **Which connector actions ask, which run silently, and which can never be
  ...
  [Apps, devices & MCP](connectors.md), with the full manifest reference in the
