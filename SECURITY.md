...
- Unauthenticated page requests → `303` redirect to `/login` (or `/setup` on a
  fresh install); API/media/app requests → `401` JSON.
...
- Expect an acknowledgement within a few days. Coordinated disclosure: we'll agree
  on a fix + disclosure timeline before any public write-up.
